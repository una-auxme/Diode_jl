...
Diode_jl is a software tool that converts the **compiled representation produced by the Dymola compiler** into **readable, executable Julia source code**. The goal is to make compiled model behavior visible and editable at the equation level, and to enable transparent numerical simulation, detailed debugging, and integration with Scientific Machine Learning workflows in Julia.



## Why the name Diode_jl?

The name **Diode_jl** combines the practical origin and the mathematical focus of the project. The work was initially motivated by simulation challenges in vehicle electrical power supply models, where diode-related behavior contributed to numerical difficulties. At the same time, the embedded acronym **ODE** highlights the core objective of the translator: converting the compiled ordinary differential equation representation generated by Dymola into readable and executable Julia code.
...
Please refer to the `Modelica_Examples` directory for a dedicated step-by-step guide for the example model `TranslatorModelle_Paper.mo`, including the required Dymola settings, artifact preparation, and execution of the translator.
...
<p align="center">
  ...
  <br>
  <em>Figure: Repository structure of Diode_jl translator.</em>
</p>
