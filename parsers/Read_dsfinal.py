    ...
    with open(file_path) as file:
        ...
        for i, line in enumerate(lines):
            if "double experiment(" in line:
                ...
                while counter < 7:
                    try:
                        simulation_settings[labels[counter]] = float(
                            lines[i + form_start].split("#")[0].strip()
                        )
                        counter += 1
                    except ValueError:
                        pass
            ...
            if line.startswith("double initialValue"):
                ...
                if len(val_plus_var) > 1:
                    ...
                    if float(values[0]) > 0:
                        ...
                        values[0] = type_of_initial_value_mapping.get(
                            int(values[0]), values[0]
                        )
                    category_of_variable = int(values[4])
                    values[4] = category_of_variable_mapping.get(
                        category_of_variable, values[4]
                    )
                    variable_name = (
                        line.split("#")[1].strip() if "#" in line else "cound not read"
                    )
                    values.append(variable_name)
                    new_row = pd.DataFrame(
                        [values], columns=column_names
                    )  # mod lines, due to package version
